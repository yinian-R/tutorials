...
CERT_AUTH_FILE="$CERT_OUTPUT_PATH/ca-cert.crt"
DAYS_VALID=365
...
echo "2. 创建CA......"
openssl req -new -x509 -keyout "$CERT_OUTPUT_PATH/ca-key.key" -out "$CERT_AUTH_FILE" -days "$DAYS_VALID" \
  -passin pass:"$PASSWORD" -passout pass:"$PASSWORD" \
  -subj "/C=$C/ST=$ST/L=$L/O=$O/OU=$OU/CN=$CN"

echo "3. 添加CA文件到broker truststore......"
keytool -storetype PKCS12 -keystore "$TRUST_STORE" -alias CARoot \
  -importcert -file "$CERT_AUTH_FILE" -storepass "$TRUST_STORE_PASSWORD" -keypass "$TRUST_KEY_PASSWORD" -noprompt
...
echo "6. 使用CA签发证书......"
openssl x509 -req -CA "$CERT_AUTH_FILE" -CAkey "$CERT_OUTPUT_PATH/ca-key.key" -in "$CERT_OUTPUT_PATH/server-cert-file" \
  -out "$CERT_OUTPUT_PATH/server-cert-signed" -days "$DAYS_VALID" -CAcreateserial -passin pass:"$PASSWORD"

openssl x509 -req -CA "$CERT_AUTH_FILE" -CAkey "$CERT_OUTPUT_PATH/ca-key.key" -in "$CERT_OUTPUT_PATH/client-cert-file" \
...
echo "9. 删除临时文件......"
# 检查文件是否存在再删除
[ -f "$CERT_OUTPUT_PATH/ca-cert.srl" ] && rm "$CERT_OUTPUT_PATH/ca-cert.srl"
[ -f "$CERT_OUTPUT_PATH/server-cert-signed" ] && rm "$CERT_OUTPUT_PATH/server-cert-signed"
[ -f "$CERT_OUTPUT_PATH/client-cert-signed" ] && rm "$CERT_OUTPUT_PATH/client-cert-signed"
[ -f "$CERT_OUTPUT_PATH/server-cert-file" ] && rm "$CERT_OUTPUT_PATH/server-cert-file"
[ -f "$CERT_OUTPUT_PATH/client-cert-file" ] && rm "$CERT_OUTPUT_PATH/client-cert-file"

echo "SSL证书生成完成！证书保存在: $CERT_OUTPUT_PATH"
